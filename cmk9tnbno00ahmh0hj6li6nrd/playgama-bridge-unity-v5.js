       if (/iPhone|iPad|iPod|Android/i.test(navigator.userAgent)) {
        ...
        bridgeScript.src = 'https://cdn.jsdelivr.net/gh/greeniYT/bs@main/bridge.playgama.com/v1/stable/playgama-bridgev2.js'
        ...
        // Function to fetch and combine .part files (starting from .part1)
        async function fetchWasmParts(baseUrl, maxParts = 20) {
            const parts = [];
            
            console.log('Starting to fetch WASM parts from:', baseUrl);
            
            // Start from part 1 (not part 0)
            for (let i = 1; i <= maxParts; i++) {
                const partUrl = `${baseUrl}.part${i}`;
                try {
                    console.log(`Fetching part ${i} from: ${partUrl}`);
                    const response = await fetch(partUrl);
                    
                    if (!response.ok) {
                        // If we get a 404, we've reached the end of parts
                        if (response.status === 404) {
                            console.log(`Part ${i} not found (404), ending part fetch`);
                            break;
                        }
                        throw new Error(`Failed to fetch part ${i}: ${response.status}`);
                    }
                    
                    const arrayBuffer = await response.arrayBuffer();
                    const uint8Array = new Uint8Array(arrayBuffer);
                    parts.push(uint8Array);
                    console.log(`✓ Part ${i} loaded successfully: ${arrayBuffer.byteLength} bytes`);
                } catch (error) {
                    // If fetch fails (likely 404), we've reached the end
                    console.log(`Error fetching part ${i}:`, error.message);
                    if (i === 1) {
                        throw new Error('No .part files found at all');
                    }
                    break;
                }
            }
            ...
            if (parts.length === 0) {
                throw new Error('No WASM parts were successfully loaded');
            }

            console.log(`✓ Successfully loaded ${parts.length} WASM parts`);

            // Combine all parts into a single Uint8Array
            const totalLength = parts.reduce((sum, part) => sum + part.length, 0);
            console.log(`Total combined size: ${totalLength} bytes`);
            ...
            // Create a Blob URL from the combined data
        ...
        function initializeBridge() {
            ...
            bridge
                ...
                .then(async () => {
                    ...
                    // Base URL for the WASM file
                    const wasmBaseUrl = 'https://cdn.jsdelivr.net/gh/greeniYT/bs@main/cmk9tnbno00ahmh0hj6li6nrd/Build/44a2eaf1f3446fe57744d7c21a4f7097.wasm.br';
                    
                    let codeUrl;
                    try {
                        console.log('Attempting to load WASM from .part files...');
                        codeUrl = await fetchWasmParts(wasmBaseUrl);
                        console.log('✓ Successfully loaded WASM from .part files');
                    } catch (partError) {
                        // Fallback to single file if .parts don't exist
                        console.error('Failed to load from parts:', partError.message);
                        console.log('Falling back to single WASM file');
                        codeUrl = wasmBaseUrl;
                    }
                    ...
                    unityLoader.onload = () => {
                        createUnityInstance(
                            ...
                            .catch((error) => {
                                console.error('Unity instantiation error:', error)
                            })
                    }
                    document.body.appendChild(unityLoader)
                })
                .catch(error => console.error('Bridge initialization error:', error))
        }
